...
protocol GetServerWorkerProtocol: AnyObject {
    func getUsers(for path: String, completion: @escaping (Result<[User], ErrorType>) -> Void)
}

final class GetServerWorker: GetServerWorkerProtocol {
    func getUsers(for path: String, completion: @escaping (Result<[User], ErrorType>) -> Void) {
        NetworkRouter(session: .shared).performRequest(for: UserEndpoint.getUsers(customPath: path), decodingType: [User].self) { result in
